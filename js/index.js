...
const textEditoptions = () => {
  ...
  const activeSecondbutton = (active, parentElement) => {
    ...
    `;
      ...
      const newButton = parentElement.querySelector(".--eensubtitle");
  ...
  const secondbuttonlistener = (button) => {
    if (button !== null) {
      if (button.classList.contains("--eensubtitle")) {
        button.addEventListener("click", (event) => {
          event.preventDefault();
  ...
  hasactiveContainer.forEach((buttons) => {
    ...
    if (toggleTarget === "--free-made-teksten") {
      ...
      h1editable.addEventListener("click", () => {
        if (subtitel !== null) {
          subtitel.style.pointerEvents = "auto";
          subtitel.style.cursor = "pointer";
        }
        textoptionbuttons.forEach((button, index) => {
          ...
          if (
            ...
            button.parentElement.id === "secondbtn"
          ) {
      ...
      ptags.forEach((fontvalue, index) => {
        ...
        if (index === 1) {
          const middlecontent = document.querySelectorAll(
            ".--middletext-content p, .--lowertext-content p, .--upper-text p, .--middletext-content input::placeholder,.--middletext-content input  "
          );
